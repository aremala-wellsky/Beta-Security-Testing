...
-- Gather Data
CREATE TABLE IF NOT EXISTS public.qlik_answer_vis_array_entry(
  visibility_id serial PRIMARY KEY,
  allow_ids integer[],
  deny_ids integer[]
);

CREATE TABLE IF NOT EXISTS public.qlik_answer_vis_provider_entry(
  visibility_id integer,
  provider_id integer
);

CREATE OR REPLACE FUNCTION public.qlik_get_vis_link_entry(
    allowvg integer[],
    denyvg integer[])
  RETURNS integer AS
$BODY$
        ...
        BEGIN
            ...
            _visibility_id := (SELECT visibility_id FROM qlik_answer_vis_array_entry WHERE allow_ids = allowvg AND deny_ids = denyvg);

            IF (_visibility_id IS NULL) THEN
                INSERT INTO qlik_answer_vis_array_entry (allow_ids, deny_ids)
                VALUES (allowvg, denyvg)
                RETURNING visibility_id INTO _visibility_id;

                INSERT INTO qlik_answer_vis_provider_entry (visibility_id, provider_id)
                SELECT _visibility_id, provider_id
                FROM (
                  SELECT provider_id
                  FROM sp_visibility_group_provider_tree vgpt 
                  WHERE vgpt.visibility_group_id = ANY(allowvg)
                  AND EXISTS (SELECT 1 FROM tmp_table_sec_non_support_entry t WHERE vgpt.provider_id = t.provider_id)
                  EXCEPT
                  SELECT provider_id
                  FROM sp_visibility_group_provider_tree vgpt
...
BEGIN
    ...
    CREATE TABLE qlik_answer_access_entry AS
    SELECT DISTINCT ON (a.answer_id, q.question_type_code, q.virt_field_name, a.client_id, a.covered_by_roi, a.provider_id, a.date_effective, i.answer_id)
    ...
    SELECT answer_id
    FROM (select DISTINCT ON (client_id, question_id) answer_id
          FROM da_answer a JOIN tmp_table_sec_aa_entry t USING (provider_id)
          WHERE a.active AND a.date_added > $1::DATE
          ORDER BY client_id, question_id, date_effective desc, answer_id desc) t
    ...
    SELECT answer_id
    FROM (select DISTINCT ON (client_id, question_id) answer_id
          FROM da_answer a JOIN tmp_table_sec_cm_entry t USING (provider_id)
          WHERE a.active AND a.date_added > $1::DATE
          ORDER BY client_id, question_id, date_effective desc, answer_id desc) t
    ) i ON (a.answer_id = i.answer_id)

    -- TODO: Verify that this EE linkage and 3 year cut off is right
    JOIN (SELECT entry_exit_id, client_id, entry_date, exit_date, provider_id
    ...
    ON (ee.client_id = a.client_id AND a.date_effective::DATE <= ee.entry_date::DATE) 

    WHERE a.active
    AND (i.answer_id IS NOT NULL OR covered_by_roi) -- Remove non-roi rows
    ORDER BY a.answer_id, q.question_type_code, q.virt_field_name, a.client_id, a.covered_by_roi, a.provider_id, a.date_effective, i.answer_id, ee.entry_exit_id DESC;

    -- Create primary key index
    ALTER TABLE qlik_answer_access_entry ADD PRIMARY KEY (answer_id);

    -- Remove all records with no inherent or explicit visiblity
    DELETE FROM qlik_answer_access_entry qaa 
    ...
    WITH global_open AS (
    SELECT qlik_get_vis_link_entry(ARRAY[0], NULL) AS visibility_id
    )
    UPDATE qlik_answer_access_entry qaa
    ...
    UPDATE qlik_answer_access_entry qaa
    SET visibility_id = (SELECT qlik_get_vis_link_entry(array_agg(CASE WHEN cav.visible THEN cav.visibility_group_id ELSE NULL END), 
    ...
    DELETE FROM qlik_answer_access_entry WHERE has_inherent_vis = FALSE AND visibility_id IS NULL;
...
SELECT qlik_build_answer_access_entry('2015-01-01', '2015-01-01');
