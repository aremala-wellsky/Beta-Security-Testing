CREATE OR REPLACE FUNCTION qlik_build_entry_answer_pivot_view(
...
BEGIN
    ...
    IF _create_base_table IS NOT DISTINCT FROM TRUE THEN 
        PERFORM qlik_build_entry_answers_table($1, $2);
    END IF;
...
FROM (
    ...
    DROP MATERIALIZED VIEW IF EXISTS qlik_entry_answer_pivot_view;
    EXECUTE 'CREATE MATERIALIZED VIEW qlik_entry_answer_pivot_view AS '||_final_query;
    RAISE NOTICE 'Finished creating qlik_entry_answer_pivot_view %',clock_timestamp();
    
    ALTER TABLE qlik_entry_answer_pivot_view OWNER TO sp5user;
END;
$BODY$
